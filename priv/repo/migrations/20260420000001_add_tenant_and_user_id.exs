defmodule BotArmyFeeds.Repo.Migrations.AddTenantAndUserId do
  ...
  def up do
    ...
    if not Ecto.Migration.column_exists?(:feeds, :tenant_id) do
      ...
      execute("UPDATE feeds SET tenant_id = '#{default_tenant_id}'::uuid WHERE tenant_id IS NULL")
    end

    # Add tenant_id and user_id to articles (idempotent)
    if not Ecto.Migration.column_exists?(:articles, :tenant_id) do
  ...
  def down do
    ...
    drop(index(:feeds, [:tenant_id]))
    drop(index(:feeds, [:user_id]))
